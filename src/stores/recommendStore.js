...
export const useRecommendStore = defineStore('recommend', {
    state: () => ({
        ...
        collaborativeRecommendations: [], // 协同过滤推荐
        similarUsersRecommendations: [], // 相似用户推荐
        currentPage: 1,
        pageSize: 10,
        loading: false,
        error: null
    ...
    actions: {
        ...
        async getCollaborativeRecommendations() {
            try {
                this.loading = true;
                const res = await getCollaborativeFilteringRecommendationsAPI();
                if (res.code === 0 && res.data) {
                    this.collaborativeRecommendations = res.data.list || [];
                }
                return res.data.list || [];
            } catch (error) {
                this.error = error.message;
                ElMessage.error('获取协同过滤推荐失败');
                throw error;
            } finally {
                this.loading = false;
            }
        ...
        async getSimilarUsersRecommendations() {
            try {
                this.loading = true;
                const res = await getSimilarUsersRecommendationsAPI();
                if (res.code === 0 && res.data) {
                    this.similarUsersRecommendations = res.data.list || [];
                }
                return res.data.list || [];
            } catch (error) {
                this.error = error.message;
                ElMessage.error('获取相似用户推荐失败');
                throw error;
            } finally {
                this.loading = false;
            }
        ...
        clearState() {
            ...
            this.similarUsersRecommendations = [];
            this.currentPage = 1;
            this.pageSize = 10;
            this.error = null;
            clearRecommendationsCache();
