                ...
                <div class="intro">
                    <div class="logo-wrapper">
                        <div class="rotate-container">
                            <div class="front">
                                <template v-if="!imageErrors.logo">
                                    <img :src="getImageUrl('logo.png')" alt="TravelRec Logo"
                                        @error="handleImageError" />
                                </template>
                                <div v-else class="icon-placeholder">
                                    <el-icon :size="60" color="#2196F3">
                                        <Place />
                                    </el-icon>
                                </div>
                            </div>
                ...
                <div class="features">
                    <div class="feature" v-for="(feature, index) in features" :key="index">
                        <div class="feature-image" :style="{ backgroundColor: '#f0f0f0' }">
                            <template v-if="!imageErrors[`feature${index + 1}`]">
                                <img :src="getImageUrl(`feature${index + 1}.png`)" :alt="feature.title"
                                    @error="handleImageError($event, `feature${index + 1}`)" />
                            </template>
                            <div v-else class="icon-placeholder">
                                <el-icon :size="40" color="#2196F3">
                                    <component :is="getFeatureIcon(index)" />
                                </el-icon>
                            </div>
                        </div>
                        <h3>{{ feature.title }}</h3>
                ...
                <div class="tech-stack">
                    <div class="tech-item" v-for="(tech, index) in techStack" :key="index">
                        <div class="tech-icon" :style="{ backgroundColor: '#f0f0f0' }">
                            <template v-if="!imageErrors[tech.icon]">
                                <img :src="getImageUrl(`${tech.icon}.png`)" :alt="tech.name"
                                    @error="handleImageError($event, tech.icon)" />
                            </template>
                            <div v-else class="icon-placeholder">
                                <el-icon :size="30" color="#2196F3">
                                    <component :is="getTechIcon(tech.name)" />
                                </el-icon>
                            </div>
                        </div>
                        <span>{{ tech.name }}</span>
...
/**
 * 关于页面组件
 * @description 展示应用的介绍、功能、技术栈、团队和联系方式
 */
 ...
 * @param {string} imageKey - 图片键名
 */
const handleImageError = (event, imageKey = 'logo') => {
    imageErrors.value[imageKey] = true;
    console.log(`图片加载失败: ${imageKey}`);
};

/**
 * 处理头像加载错误
 * @param {Event} event - 错误事件
 * @param {number} index - 团队成员索引
 */
const handleAvatarError = (event, index) => {
    // 标记该成员头像加载失败
    teamMembers.value[index].hasError = true;
    // 设置默认头像
    teamMembers.value[index].avatar = getImageUrl('default-avatar.png');
    // 如果默认头像也不存在，可以使用在线占位图服务
    if (!teamMembers.value[index].avatar) {
        teamMembers.value[index].avatar = 'https://via.placeholder.com/80?text=用户';
    }
};
...
const getImageUrl = (name) => {
    // 尝试从assets目录加载
    try {
        return new URL(`../../assets/about/${name}`, import.meta.url).href;
    } catch (error) {
        // 如果assets目录不存在，尝试从public目录加载
...
/**
 * 获取功能图标
 * @param {number} index - 功能索引
 * @returns {string} 图标组件名称
 */
const getFeatureIcon = (index) => {
    const icons = ['Compass', 'Calendar', 'Share'];
    return icons[index] || 'Place';
};
...
const getTechIcon = (techName) => {
    const iconMap = {
        'Vue3': 'Monitor',
        'Element Plus': 'Connection',
        'Pinia': 'DataAnalysis'
    };
    return iconMap[techName] || 'Monitor';
};
...
const teamMembers = ref([
    {
        avatar: getImageUrl('favicon.ico'),
        name: '张三',
        role: '项目前后端负责人'
    },
    {
